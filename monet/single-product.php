...
$context = [
    'skip' => ($is_finished_model || $is_cannot_sell),
];

// ----------------- supplier 判定 -----------------
$supplier = [
    'ofm' => has_term('s-ofm', 'supplier'),
    'amp_group' => has_term(
        ['s-amp','s-sanby','s-artlink','s-total-m','s-jsr','s-poa','s-teeya-phaiboon','s-sus'],
        'supplier'
    )
...
// ----------------- 在庫定義 -----------------
$stock = [
    'monet'    => (int) get_field('monet_stock'),
    'th'       => (int) get_field('th_stock'),
    'ktw'      => (int) get_field('ktw_stock'),
    'sis'      => (int) get_field('sis_stock'),
...
// 即納・総在庫
$stock['fast_total']  = $stock['monet'] + $stock['thai_total'];
$stock['grand_total'] = $stock['fast_total'] + $stock['jp'];

// ----------------- ターム判定 -----------------
$flag = [
    'thai_purchase' => has_term('thai-purchase', 'import_method'),
    'toriyose'      => false,
];

// 121693（取り寄せ）
$terms = get_the_terms(get_the_ID(), 'trusco_status');
if (!is_wp_error($terms)) {
    foreach ($terms as $t) {
        if ((int) $t->term_id === 121693) {
            $flag['toriyose'] = true;
            break;
        }
    }
...
function render_stock_item($label, $qty, $text) {
    ...
    if ($qty === 0) {
        return '<span class="stock-non stock-style">0</span> ' . $text;
    }

    $class = ($qty <= 4) ? 'stock-low' : 'stock-high';

    return '<span class="' . $class . ' stock-style">'
        . $formatted
        . '</span> ' . $text;
}
?>
...
<?php elseif ($supplier['amp_group']) : ?>

<tr>
    <th>สต็อก</th>
    <td>
        <?php if ($flag['toriyose']) : ?>
            <span class="stock-non stock-style">0</span> (สินค้าต้องพรีออเดอร์)
        <?php else : ?>
            <span class="stock-high stock-style">100</span> (สต็อกในไทย)
        <?php endif; ?>
    </td>
</tr>
...
<tr>
    ...
    <td>
        <?php if ($flag['toriyose'] && get_field('lead_time')) : ?>
            ประมาณ <?php the_field('lead_time'); ?> วันทำการ
        <?php else : ?>
...
<tr>
    ...
    <td>
        ...
        <?php elseif ($stock['grand_total'] === 0) : ?>

            <span class="stock-maker stock-style">โปรดติดต่อเจ้าหน้าที่</span>

        <?php elseif ($leadtime['jp_days'] === null) : ?>

            <div>ไม่เกิน <?= $stock['fast_total']; ?> ชิ้น : 3 วันทำการ</div>
            <div><?= ($stock['fast_total'] + 1); ?> ชิ้นขึ้นไป : โปรดติดต่อเจ้าหน้าที่</div>

        <?php else : ?>

            <?php if ($stock['fast_total'] > 0) : ?>
                <div>ไม่เกิน <?= $stock['fast_total']; ?> ชิ้น : 3 วันทำการ</div>
            <?php endif; ?>

            <?php if ($stock['jp'] > 0) : ?>
                <div>
                    ไม่เกิน <?= ($stock['fast_total'] + $stock['jp']); ?> ชิ้น :
                    <?= $leadtime['jp_days']; ?> วันทำการ
                </div>
            <?php endif; ?>

            <div>
                <?= ($stock['fast_total'] + $stock['jp'] + 1); ?> ชิ้นขึ้นไป :
