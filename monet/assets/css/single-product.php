...
<?php elseif ( 
    ...
    <?php endif; ?>
<?php elseif ( has_term('s-tkg', 'supplier' ) ) : ?>
<tr>
	<th>สต็อกในญี่ปุ่น</th>
	<td>
		<?php
		$tkg_stock = (int) get_field('tkg_stock');
		$formatted = number_format( $tkg_stock );
		if ( $tkg_stock === 0 ) {
		    echo '0';
		} elseif ( $tkg_stock >= 1 && $tkg_stock <= 4 ) {
 ...
 * 数量 → (สต็อกใน XXX)
 * -------------------------------------- */
function render_stock_item($label, $stock) {
    $stock_num = (int) $stock;
...
// 1. Monet在庫がある場合
    ...
    echo render_stock_item('Monet', $monet_stock) . '<br>';
    echo render_stock_item('ไทย', $thai_stock);

    if (!$is_thai_purchase) {
        echo '<br>' . render_stock_item('ญี่ปุ่น', $jp_stock);
    }
...
// 4. 全在庫なし（finished-model 除外）
    ...
    if (!$is_thai_purchase) {
        echo '<br>' . render_stock_item('ญี่ปุ่น', 0);
    }
}
